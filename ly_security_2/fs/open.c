...
PRIVATE int alloc_smap_bit(int dev, int nr_sects_to_alloc)
{
	...
	for (i = 0; i < sb->nr_smap_sects; i++) { /* smap_blk0_nr + i :
		...
		for (j = 0; j < SECTOR_SIZE && nr_sects_to_alloc > 0; j++) {
			/* find a free bit */
			for (k = 0; k < 8; k++) {
				if (((fsbuf[j] >> k) & 1) == 0) { /* found a free bit */
					/* check if we can allocate nr_sects_to_alloc bits */
					int m;
					int valid = 1;
					int t_j = j;
					int t_k = k;
					for (m = 0; m < nr_sects_to_alloc; m++) {
						if (t_j >= SECTOR_SIZE) {
							valid = 0;
							break;
						}
						if (((fsbuf[t_j] >> t_k) & 1) != 0) {
							valid = 0;
							break;
						}
						t_k++;
					...
					if (valid) {
						/* free bit found, calculate free_sect_nr */
						/* preserving original formula: k - 1 */
